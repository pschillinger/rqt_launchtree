...
class LaunchtreeEntryItem(QTreeWidgetItem):
    _type_order = [dict, roslaunch.core.Node, LaunchtreeRosparam,
                   roslaunch.core.Param, LaunchtreeRemap, LaunchtreeArg, object]
    #inconsistent = False
    ...
    def __ge__(self, other):
        own_type_idx = [isinstance(self.instance, t)
                        for t in self._type_order].index(True)
        other_type_idx = [isinstance(other.instance, t)
                          for t in self._type_order].index(True)
...
class LaunchtreeWidget(QWidget):
    ...
    def __init__(self, context):
        ...
        res_folder = os.path.join(
            self._rp.get_path('rqt_launchtree'), 'resource')
        ...
        self._properties_empty_ui = os.path.join(
            res_folder, 'properties_empty.ui')
        self._properties_param_ui = os.path.join(
            res_folder, 'properties_param.ui')
        ...
        self._icon_rosparam = QIcon(os.path.join(
            res_folder, 'img/rosparam_load.png'))
    ...
    def load_launchfile(self):
        ...
        launchargs = roslaunch.substitution_args.resolve_args(
            self.args_input.text()).split(' ')
        if os.path.isfile(filename):
            self.progress_bar.setRange(0, 0)
            self._load_thread = threading.Thread(
                target=self._load_launch_items, args=[filename, launchargs])
    ...
    def _load_launch_items(self, filename, launchargs):
        ...
        try:
            ...
            loader.load(filename, self._launch_config,
                        verbose=False, argv=['', '', ''] + launchargs)
            items = self.display_config_tree(self._launch_config.tree)
        except Exception as e:
            error_msg = re.sub(r'(\[?(?:/\w+)+\.launch\]?)',
                               lambda m: '[%s]' % self._filename_to_label(
                                   m.group(0)),
                               str(e)
                               )
            help_msg = ''
    ...
    def display_config_tree(self, config_tree):
        ...
        for key, instance in list(config_tree.items()):
            ...
            if isinstance(i.instance, dict):
                ...
                i.setIcon(
                    0, self._icon_include if not i.inconsistent else self._icon_warn)
            else:
    ...
    def update_package_list(self):
        self._package_list = sorted(
            [p for p in self._rp_package_list if len(
                self._get_launch_files(self._rp.get_path(p))) > 0]
        )
    ...
    def launch_entry_changed(self, current, previous):
        ...
        if isinstance(data, roslaunch.core.Param):
            ...
            if isinstance(data.value, list):
                ...
                self.param_value_list.addItems(
                    list(str(v) for v in data.value))
            ...
            self.node_prefix.setText(
                str(data.launch_prefix) if data.launch_prefix is not None else '')
            self.node_prefix.setEnabled(data.launch_prefix is not None)
            self.node_machine.setText(
                str(data.machine_name) if data.machine_name is not None else '')
            ...
            self.arg_value.setText(
                str(data.value) if data.value is not None else '')
            self.arg_default.setText(
                str(data.default) if data.default is not None else '')
            ...
            self.machine_user.setText(
                str(data.user) if data.user is not None else '')
            self.machine_user.setEnabled(data.user is not None)
            self.machine_loader.setText(
                str(data.env_loader) if data.env_loader is not None else '')
            ...
            path_segments = self.launch_view.currentItem().text(
                0).split(self._launch_separator)
            ...
            self.properties_content.setCurrentIndex(3)
            (p, l) = self.launch_view.currentItem().text(
                0).split(self._launch_separator)
            (d, f) = os.path.split(l)
            ...
            if search_text:
                ...
                if isinstance(entry.instance, roslaunch.core.Param):
                    ...
                    if entry.instance.value:
                        search_text_contained_in_meta |= search_text in str(
                ...
                show &= (search_text in entry.text(
                    0)) or search_text_contained_in_meta

            if show:
                entry.setBackgroundColor(
                    0, self._highlight_color if highlight else self._neutral_color)

            if entry.childCount() > 0:
                not_empty = any(map(filter_launch_entry, list(
                    map(entry.child, list(range(entry.childCount()))))))
                show |= show_empty or not_empty
                entry.setExpanded(not collapse and (
                    expand or entry.isExpanded()))
