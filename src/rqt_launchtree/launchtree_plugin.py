...
class LaunchtreePlugin(Plugin):
    ...
    def save_settings(self, plugin_settings, instance_settings):
        ...
        instance_settings.set_value(
            self._SETTING_LASTLAUNCHFILE, self._widget.launchfile_select.currentText())
        instance_settings.set_value(
            self._SETTING_LASTLAUNCHARGS, self._widget.args_input.text())

    def restore_settings(self, plugin_settings, instance_settings):
        self._widget.editor = instance_settings.value('editor', 'gedit')
        self._widget.args_input.setText(
            instance_settings.value(self._SETTING_LASTLAUNCHARGS, ''))
        pkg_idx = self._widget.package_select.findText(
            instance_settings.value(self._SETTING_LASTPKG))
        if pkg_idx >= 0:
            ...
            launch_idx = self._widget.launchfile_select.findText(
                instance_settings.value(self._SETTING_LASTLAUNCHFILE))
            if launch_idx >= 0:
